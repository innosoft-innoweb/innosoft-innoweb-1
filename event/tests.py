...
class EventDetailViewTest(StaticLiveServerTestCase):
    ...
    def test_participants_in_podium(self):
        ...
        for event in events:
            ...
            if(len(scores) > 0):
                for i in range(3):
                    username = self.browser.find_elements(By.CLASS_NAME, 'podium-item')[i].find_element(By.CLASS_NAME, 'podium-winner-wrapper').find_element(By.CLASS_NAME, 'podium-winner').text
                    photo = self.browser.find_elements(By.CLASS_NAME, 'podium-item')[i].find_element(By.CLASS_NAME, 'podium-winner-wrapper').find_element(By.CLASS_NAME, 'podium-photo').get_attribute("src")
                    position = self.browser.find_elements(By.CLASS_NAME, 'podium-item')[i].find_element(By.CLASS_NAME, 'podium-position').text
                    
                    self.assertEqual(username, scores[int(position)-1].participant.get_username())
