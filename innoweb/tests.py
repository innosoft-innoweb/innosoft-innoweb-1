...
class HomeViewTest(StaticLiveServerTestCase):
    ...
    def test_podium(self):
        ...
        for podium_item, winner, position in zip(podium_items, winners, positions):
            ...
            podium_position = podium_item.find_element(
                By.CSS_SELECTOR, ".podium-position"
            )
            ...
            assert podium_link.get_attribute(
                "href"
            ) == self.live_server_url + "/participante/" + str(winner.id)
    ...
    def test_open_events(self):
        ...
        event_containers = self.browser.find_elements(
            By.CSS_SELECTOR, ".events-container"
        )

        open_events_cards = event_containers[0].find_elements(
            By.CSS_SELECTOR, "a.event-card"
        )
    ...
    def test_current_events(self):
        ...
        current_events_cards = event_containers[1].find_elements(
            By.CSS_SELECTOR, "a.event-card"
        )
    ...
    def test_past_events(self):
        ...
        past_events_cards = event_containers[2].find_elements(
            By.CSS_SELECTOR, "a.event-card"
        )
    ...
    def check_event_card(cls, self, event_card, event):
        ...
        assert event_card.get_attribute(
            "href"
        ) == self.live_server_url + "/evento/" + str(event.id)
...
class RegisterViewTest(StaticLiveServerTestCase):
    ...
    def send_keys(
        self, username, firstname, lastname, email, password, password2, photo
    ):
    ...
    def test_register_username_fail(self):
        ...
        assert (
            self.browser.find_element(By.CLASS_NAME, "errorlist").is_enabled() == True
        )
...
class EventViewTest(StaticLiveServerTestCase):
    ...
    def test_register_event(self):
        ...
        assert (
            self.browser.find_element(By.CLASS_NAME, "alert-success").is_enabled()
            == True
        )
    ...
    def test_register_event_already_registered(self):
        ...
        register_button.click()
        assert (
            self.browser.find_element(By.CLASS_NAME, "alert-warning").is_enabled()
            == True
        )
    ...
    def test_register_event_without_login(self):
        ...
        register_button.click()
        assert (
            self.browser.find_element(By.CLASS_NAME, "alert-danger").is_enabled()
            == True
        )
...
class ProfileViewTest(StaticLiveServerTestCase):
    ...
    def test_next_events_are_shown(self):
        ...
        register_button = self.browser.find_element(By.CSS_SELECTOR, "a.sign-up")
        register_button.click()
        profile_url = self.browser.find_element(
            By.CSS_SELECTOR, "a.navbar-link-primary"
        ).get_attribute("href")
        self.browser.get(profile_url)
        assert (
            self.browser.find_element(By.CSS_SELECTOR, "td.event-name").is_enabled()
            == True
        )
