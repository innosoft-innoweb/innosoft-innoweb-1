...
def get_future_events(request):
    user = request.user
    future_events = (
        Score.objects.filter(participant=user)
        .filter(value=None)
...
def profile(request, id):
    def get_position(participant, event):
        scores = (
            Score.objects.filter(event=event).order_by("-value").exclude(value=None)
        )
        position = 1
        for score in scores:
    ...
    events_aux = (
        Score.objects.filter(participant=p).order_by("-value").exclude(value=None)
    )
